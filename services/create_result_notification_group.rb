module ResultNotifications
  module Services
    class CreateResultNotificationGroup < Service

      # This service creates a ResultNotificationGroup
      ...
      def call(params)
        ...
        result_notification_group = ResultNotificationGroup.create!(
      ...
      def authorise_user
        ...
        if authoriser.client_user?
          errors << "You have insufficient permissions to create a result notification group" unless authoriser.has_client_permission?("manage_notifications")
        end

        if authoriser.laboratory_user?
          errors << "You have insufficient permissions to create a result notification group" unless authoriser.authorise?("manage_client_notifications")
        end
